...
Here, we provide guides for evaluating the trained automatic coding model.
...
* [`-l`, `--load_model_type`]: Choose one of [`metric`, `loss`, `last`].
    * `metric` (default): Resume the model with the best validation set's metric such as AUCROC, AUCPRC, etc.

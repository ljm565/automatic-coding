...
from transformers import MistralModel, MistralPreTrainedModel, MistralForCausalLM, MistralConfig
from transformers.models.mistral.modeling_mistral import MistralDecoderLayer, MistralRMSNorm, MistralMLP
...
from .bi_mistral_modules import MISTRAL_ATTENTION_CLASSES
...
class MistralBiModel(MistralModel):
    ...
    def forward(
        ...
        if self.gradient_checkpointing and self.training:
            if use_cache:
                LOGGER.warning(
                    colorstr("yellow", "`use_cache=True` is incompatible with gradient checkpointing. Setting `use_cache=False`...")
                )
                use_cache = False
